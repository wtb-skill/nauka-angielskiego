...
class FileInitController:
    ...
    STARTER_SHA256_SUM = (
        "b138cbdcc4501f6eb769811c71408392144569281815116c7a60c31bb49e1f95"
    )
    ...
    def __init__(self):
        ...
        self.starter_results = FileInitModel(
            path=self.starter_path,
            exist=True,
            checksums=True,
        )
        ...
        self.to_learn_results = FileInitModel(
            path=self.to_learn_path,
            exist=True,
        )
        ...
        self.hand_results = FileInitModel(
            path=self.hand_path,
            exist=True,
            json_length=True,
            json_key=True,
        ...
        self.mastered_results = FileInitModel(
            path=self.mastered_path,
            exist=True,
            json_length=True,
        )
        ...
        self.settings_results = FileInitModel(
    ...
    def check_starter_checksum(self) -> bool:
        ...
        for starter_sum in [
            a for a in dir(self) if a.startswith("STARTER") and a.endswith("SUM")
        ]:
            # if any of hard-coded class attributes sums are not equal to calculated one return False
            if not getattr(self, starter_sum) == self.starter_results.checksums.get(
                starter_sum.split("_")[1].lower()
            ):
                return False
        return True
    ...
    def check_hand_stars_count(self):
        if self.hand_results.exist:
            ...
            try:
                ...
                if (
                    min(_stars_count) >= self.HAND_MIN_STARS
                    and max(_stars_count) <= self.HAND_MAX_STARS
                ):
                    return True
            except ValueError:
                pass
    ...
    def check_first_run(self):
        ...
        if self.starter_exist and self.starter_checksum_ok:
            if all(
                ...
                FileInitView(
                    logging_on_screen=self.LOGGING_ON_SCREEN,
    ...
    def save_results(self):
        ...
        _results["CHECKSUM"] = {
            "sha1": hashlib.sha1(f"{_results}".encode()).hexdigest()
        }
